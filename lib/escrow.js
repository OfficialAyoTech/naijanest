...
//
// IMPORTANT: `transferData.status` (a boolean) only means Paystack ACCEPTED
// the transfer request — it does NOT mean the transfer actually completed.
// The real outcome is in `transferData.data.status` ('success', 'otp',
// 'pending', etc.), and this function only returns successfully for
// 'success'. This distinction matters a lot: if the Paystack account has
// "Confirm transfers before sending" (Transfer OTP) turned on, every
// transfer gets accepted (status: true) but then sits at data.status:
// 'otp' waiting for a confirmation nothing in this codebase can ever give
// it, and eventually expires as "abandoned" with no money moved. Treating
// API-acceptance as completion caused NaijaNest escrow 0f02aea5 to show as
// "released" in the admin dashboard on Sept 26, 2026 while Paystack's own
// Transfers page showed zero transfers sent — this fix exists specifically
...
// otpRequired means the transfer is stuck on a setting a human must fix —
// NOT self-resolving, unlike insufficientBalance/transferPending. transferPending
// means Paystack accepted it and is still processing it — safe to treat like
// insufficientBalance (retry later, don't alert loudly).
export async function transferToPayee({ payee, amount, refBase, reason }) {
  ...
  // The API call was accepted — but that is NOT the same as the transfer
  // having actually completed. Check the real status Paystack put in the
  // response before treating this as done.
  const realStatus = transferData.data && transferData.data.status;
  ...
  if (realStatus === 'otp') {
    const err = new Error(
      `Transfer accepted but stuck awaiting OTP confirmation (transfer_code: ${transferData.data.transfer_code}). ` +
      `This means the Paystack account's "Confirm transfers before sending" setting is turned on, which blocks ` +
      `every automated payout — check Settings > Preferences > Transfers > OTP confirmation and turn it off.`
    );
    err.otpRequired = true;
    throw err;
  }

  if (realStatus === 'pending' || realStatus === 'processing' || realStatus === 'queued') {
    const err = new Error(
      `Transfer accepted but still ${realStatus} on Paystack's side (transfer_code: ${transferData.data.transfer_code}). ` +
      `Not yet confirmed as sent.`
    );
    err.transferPending = true;
    throw err;
  }

  // Anything else (failed, reversed, or a status we don't recognize) — treat
  // as a real rejection rather than silently succeeding.
  const err = new Error(
    `Transfer accepted by API but ended in unexpected status "${realStatus}" (transfer_code: ${transferData.data && transferData.data.transfer_code}).`
  );
  err.paystackRejected = true;
  throw err;
}
...
export async function releaseEscrow({ escrow, headers }) {
  ...
  // transferToPayee() below only returns successfully when the transfer's
  // REAL status is 'success' (see its own comments) — so everything from
  // this point on only runs once money has genuinely moved, never just on
  // API acceptance.
...
export async function runListerPayoutSweep({ headers }) {
  ...
  for (const row of rows) {
    try {
      ...
      if (e.insufficientBalance || e.transferPending) { summary.deferred++; continue; }
      if (e.otpRequired) {
        summary.failed++;
        await logError(`escrow-lister-otp:${row.id}`, e, { windowMinutes: 24 * 60, skipDuplicateLog: true });
        continue;
      }
      summary.failed++;
