...
export async function releaseEscrow({ escrow, headers }) {
  ...
  if (!landlord || !landlord.bank_account_number) {
    // Expected/recoverable, not a code bug — the landlord just hasn't added
    // payout details yet. Tagged the same way insufficientBalance is below,
    // so a caller (e.g. the auto-release cron) can choose to alert once and
    // then go quiet on repeat sweeps for the SAME escrow, instead of paging
    // admin every single run until the landlord finally adds their bank
    // details. See the auto-release cron for how this flag is meant to be
    // used — that's the file that actually controls the repeat noise.
    const err = new Error(`Landlord ${escrow.landlord_id} has no bank details on file`);
