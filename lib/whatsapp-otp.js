...
import { WHATSAPP_TEMPLATE_LANG } from './notify.js';
...
// Its own language setting — phone_verification_code was very possibly
// approved under a different language than escrow_notification, so this
// doesn't reuse WHATSAPP_TEMPLATE_LANG. Set OTP_TEMPLATE_LANG in Vercel if
// WhatsApp Manager shows anything other than plain "English" for it.
const OTP_TEMPLATE_LANG = process.env.OTP_TEMPLATE_LANG || 'en';
...
export async function sendWhatsAppOtp(phoneE164Digits, pin) {
  ...
  const resp = await fetch(`https://graph.facebook.com/v20.0/${process.env.WHATSAPP_PHONE_NUMBER_ID}/messages`, {
    ...
    body: JSON.stringify({
      ...
      template: {
        name: 'phone_verification_code', language: { code: OTP_TEMPLATE_LANG },
        components: [
          { type: 'body', parameters: [{ type: 'text', text: pin }] },
          // The copy-code button also carries the code as its own parameter —
          // this is what makes the "Copy code" button in WhatsApp actually work.
