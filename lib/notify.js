...
// errors.
//
// `source` doubles as the dedupe key — two calls with the same source string
// are treated as "the same problem recurring" for throttling purposes. For
// most callers that's fine as a plain label like 'paystack-webhook'. But a
// caller can also fold an identifier into it (e.g.
// `ESCROW-AUTO-RELEASE:${escrow.id}`) to get per-item throttling instead of
// one shared bucket for every occurrence of that error type — useful when a
// scheduled sweep hits the same known, unresolved issue on every run.
//
// `options.windowMinutes` (default 15) controls how long a source stays
// "recently seen" before it's treated as new again.
//
// `options.skipDuplicateLog` (default false) — when true, a source seen
// within the window is skipped entirely: no new error_logs row, no alert.
// Use this for errors that are expected to keep recurring unchanged until
// someone takes an action outside the app (e.g. a landlord adding bank
// details) — logging it once already told you everything a second, third,
// and hundredth identical row would. Leave this false (the default) for
// anything where you'd want to see every occurrence in the Metrics tab even
// if you're not re-alerted for each one.
// Language code `escrow_notification` was approved under in WhatsApp Manager
// (e.g. 'en', 'en_US', 'en_GB'). Set WHATSAPP_TEMPLATE_LANG in Vercel to change it.
export const WHATSAPP_TEMPLATE_LANG = process.env.WHATSAPP_TEMPLATE_LANG || 'en';
export async function logError(source, error, options = {}) {
  const { windowMinutes = 15, skipDuplicateLog = false } = options;
  try {
    ...
    const seenRecently = recentRows.length > 0;
    const shouldAlert = !seenRecently;

    if (seenRecently && skipDuplicateLog) {
      // Already recorded within the window and the caller doesn't want
      // repeats logged — this occurrence told us nothing new.
      return;
    }
  ...
  try {
    ...
    const resp = await fetch(`https://graph.facebook.com/v20.0/${process.env.WHATSAPP_PHONE_NUMBER_ID}/messages`, {
      ...
      body: JSON.stringify({
        ...
        template: {
          name: 'escrow_notification', language: { code: WHATSAPP_TEMPLATE_LANG },
